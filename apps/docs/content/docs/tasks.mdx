---
title: "Tasks"
description: "Tasks are one-off project commands that can use the same env, cwd, and parameter interpolation model as the rest of Zapper."
---
...
Use `{{ARGS}}` to forward extra CLI arguments with shell
quoting:
...
If commands contain literal `{{` and `}}`,
change delimiters:
