...
- Zapper describes native supervisor-managed processes and Docker-managed services in one config model and treats them as peer services. Container-only tools and process-only tools cannot honestly claim that same cross-runtime model.
...
- Zapper namespaces native processes, Docker containers, and generated volumes together under the same project/instance model. That gives one runtime identity across mixed local infrastructure.
