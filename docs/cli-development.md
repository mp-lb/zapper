...
The workflow provisions the Vercel projects/domains through Terraform, builds `@mp-lb/zapper-landing-page` and `@mp-lb/zapper-docs` for verification, then deploys each project from that project's workspace directory with the Vercel CLI.
